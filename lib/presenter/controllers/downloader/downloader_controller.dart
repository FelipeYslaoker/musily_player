...
import 'package:musily_player/core/utils/is_valid_url.dart';
...
import 'package:musily_player/presenter/controllers/downloader/downloader_methods.dart';
  ...
  DownloaderMethods defineMethods() {
    return DownloaderMethods(
      ...
      getItem: (track) {
        return data.queue
            .where(
              (e) => (e.track.hash ?? e.track.id) == (track.hash ?? track.id),
            )
            .firstOrNull;
      },
      isOffline: (track) {
          ...
          for (final item in queueCopy) {
            if (item.status != DownloadStatus.completed) {
              ...
              final invalidFilePath = path.join(
                appDir.path,
                'media/audios/${item.track.hash ?? item.track.id}.partial',
              );
              await methods.deleteDownloadedFile(
                path: invalidFilePath,
              );
              queue.removeWhere((e) =>
                  (e.track.hash ?? e.track.id) ==
                  (item.track.hash ?? item.track.id));
            }
          }
          updateData(
            data.copyWith(
      ...
      deleteDownloadedFile: ({track, path}) async {
        if (isValidDirectory((track?.url ?? path) ?? '')) {
          if (path != null) {
                  ...
                  return IconButton(
                    onPressed: () {
                      if (isPlayingOffline && isPlayingThisItem) {
                        ...
                        final index = data.queue.indexWhere((e) =>
                            (e.hash ?? e.id) ==
                            (item.track.hash ?? item.track.id));
                        if (index != -1) {
      ...
      downloadArtworks: (track) async {
        final appDir = await getApplicationSupportDirectory();
        final fileName = track.album?.id ?? track.hash ?? track.id;
        final artDir = path.join(
          appDir.path,
          'media/images/$fileName',
        );
        if (isValidUrl(track.highResImg ?? '')) {
          final artDirHigh = '$artDir-high';
          ...
          task?.status.addListener(() {
            if (task.status.value == DownloadStatus.completed) {
              track.highResImg = artDirHigh;
            }
          });
        }
        if (isValidUrl(track.lowResImg ?? '')) {
          final artDirLow = '$artDir-low';
          final task = await methods.downloadFile(
            artDirLow,
            track.lowResImg!,
          );
      ...
      addDownload: (
        ...
        if (track.url == null) {
          ...
          _gettingUrl++;
          final url = await playerController?.loadUrl?.call(track);
          if (url == null) {
        ...
        await methods.downloadArtworks(track);
        final task = await methods.downloadFile(
          downloadDir,
          track.url!,
        );
